...
void *mynewthread(void *dummy)
{
  ...
  a = a + 2;
  printf("My new thread\nThread %lu, Calculation result = %d\n", mythid, a);
  return NULL;
}
...
int main()
{
  ...
  newresult = pthread_create (&newthid, (pthread_attr_t *)NULL, mynewthread, NULL);
  ...
  printf("Thread created, newthid = %lu\n", newthid);
